...
public:
    struct Config {
        ...
        Config() 
            ...
            , tcpPort(5555)
            , udpPort(5556)
            , vrpnHost("172.26.213.1")  // Real VRPN server
            , vrpnPort(3883)
        ...
        , lastOptimizationTime_(0)
    {
        ...
        if (config_.droneAddresses.empty()) {
            for (int i = 0; i < config_.numDrones; i++) {
                // Use real drone addresses
                if (i == 0) {
                    config_.droneAddresses.push_back("172.26.209.100"); // Drone_0 (Master)
                } else if (i == 1) {
                    config_.droneAddresses.push_back("172.26.209.103"); // Drone_1
                } else {
                    config_.droneAddresses.push_back("172.26.209.100"); // Fallback to Drone_0
                }
            }
        }
        ...
        while (config_.droneAddresses.size() < (size_t)config_.numDrones) {
            config_.droneAddresses.push_back("172.26.209.100"); // Fallback to master drone
        }
...
private:
    ...
    bool InitializeUdpSocket() {
        ...
        if (!config_.droneAddresses.empty()) {
            ...
            test_addr.sin_port = htons(config_.udpPort);
            ...
            if (inet_pton(AF_INET, config_.droneAddresses[0].c_str(), &test_addr.sin_addr) <= 0) {
                ...
                if (test_result < 0) {
                    ...
                    logger.info("UDP test send successful (%zd bytes sent to %s:%d)", 
                            test_result, config_.droneAddresses[0].c_str(), config_.udpPort);
                }
            }
        }
    ...
    bool SendUnicastMessage(uint32_t sequenceNumber, int droneIndex, 
        ...
        destAddr.sin_port = htons(config_.udpPort);
