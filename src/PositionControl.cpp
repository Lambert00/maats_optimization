...
bool PositionControl::InitializeServerConnection() {
    ...
    try {
        // Get server settings from shared parameters
        std::string serverAddress = "172.26.213.70"; // Server IP address
        ...
        if (!unicastReceiver) {
            try {
                // Use fixed port 5556 for all drones (they have different IP addresses)
                int fixedPort = resultPort; // Always use DEFAULT_RESULT_PORT (5556)
                
                Info("Creating new UDP unicast socket on port %d", fixedPort);
                // Use port-only constructor to create a properly bound receiving socket
                unicastReceiver.reset(new UdpSocket(this, ObjectName() + "_unicast", fixedPort));
